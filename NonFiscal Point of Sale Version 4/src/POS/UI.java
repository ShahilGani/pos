...
public class PaymentPopulator {
    ...
    JButton btnSelect, btnSaveOrder, btnVoid, btnSearch, btnPrintRcpt, btnLastTrans, btnDiscount, btnLogoff;
    DefaultTableModel model, resultModel;
    JTable tblSalesTable, tblSearchResult;
    JTextField txtSearch, txtBcode, txtTamount, txtChange, txtTotal, txtOrderNumber, txtTableNumber;
    JLabel lblBcode, lbldepartment, lblTamount, lblChange, lblTotal, lblCustomer, lblSection, lblTableNumber, lblUsernameTxt;
		...
		public void keyPressed(KeyEvent arg0) {
			...
			if (arg0.getKeyCode()==KeyEvent.VK_ENTER){
				try {
					...
					/*BufferedImage logoImage = ImageIO.read(new File("C:\\Libraries\\company_logo")); // Replace with the correct path to your logo
			        ByteArrayOutputStream baos = new ByteArrayOutputStream();
			        ImageIO.write(logoImage, "jpg", baos);
			        byte[] logoBytes = baos.toByteArray();*/
					printerServices.printString("POS-Printer", Bold_on + " Leesha Kwik Save" + Bold_off + "\n");
	                printerServices.printString("POS-Printer", "Bvumbwe Trading Center\n");
	                printerServices.printString("POS-Printer", "P.O. Box 2695\n");
	                printerServices.printString("POS-Printer", "Blantyre\n");
	                printerServices.printString("POS-Printer", "Cell: 0 999 946 401\n");
	...
	private boolean validateUser(String username, String password) {
        ...
        try {
			...
			try (Connection con = DriverManager.getConnection(connectionUrl);
		             PreparedStatement pst = con.prepareStatement("SELECT * FROM users WHERE username = ? AND password = ? AND Accesslevel = 9")) {
			    ...
			    try {
			        ...
			        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
			            ...
			            try (Connection con = DriverManager.getConnection(connectionUrl);
							...
							stmt.executeUpdate(closeSession);
							System.out.println(closeSession);
			                // SQL query for VAT Sales
			                String SQL = "SELECT DocumentItem.Price, DocumentItem.Total, Document.Number FROM DocumentItem, Document, ProductTax " +
			                             "WHERE DocumentItem.DocumentId = Document.Id AND DocumentItem.ProductId = ProductTax.ProductId " +
			                             "AND ProductTax.TaxId = '1' AND Number LIKE '19-200%' AND Document.Date = '" + mydate + "'";
			                System.out.println(SQL);
			                try (ResultSet rs = stmt.executeQuery(SQL)) {
			                    while (rs.next()) {
			                        String vprice = rs.getString("Total");
			                        VATSales += Double.parseDouble(vprice);
			                    }
			                }

			                BigDecimal bdVATSales = BigDecimal.valueOf(VATSales).setScale(2, RoundingMode.HALF_UP);

			                // SQL query for non-VAT Sales
			                String SQL2 = "SELECT DocumentItem.Price, DocumentItem.Total, Document.Number FROM DocumentItem, Document, ProductTax " +
			                              "WHERE DocumentItem.DocumentId = Document.Id AND DocumentItem.ProductId = ProductTax.ProductId " +
			                              "AND ProductTax.TaxId = '2' AND Number LIKE '19-200%' AND Document.Date = '" + mydate + "'";
			                System.out.println(SQL2);
			                try (ResultSet rs = stmt.executeQuery(SQL2)) {
			                    while (rs.next()) {
			                        String price = rs.getString("Total");
			                        NVATSales += Double.parseDouble(price);
			                    }
			                }

			                BigDecimal bdNVATSales = BigDecimal.valueOf(NVATSales).setScale(2, RoundingMode.HALF_UP);

			                // VAT and Sales calculations
			                Double VAT = VATSales * 0.165;
			                BigDecimal bdVAT = BigDecimal.valueOf(VAT).setScale(2, RoundingMode.HALF_UP);
			                Double SalesTotal = VATSales + NVATSales;
			                BigDecimal bdSalesTotal = BigDecimal.valueOf(SalesTotal).setScale(2, RoundingMode.HALF_UP);

			                // Printing the receipt header
			                // Load and process the logo image
			                /*BufferedImage logoImage = ImageIO.read(new File("C:\\Libraries\\comp.jpg"));
			                if (logoImage != null) {
			                    // Convert to monochrome and print the logo
			                    BufferedImage monochromeLogo = printerServices.convertToMonochrome(logoImage);
			                    printerServices.printImage("POS-Printer", monochromeLogo);
			                }*/
			                ...
			                // Printing sales item list
			                String SQL3 = "SELECT Product.Name, DocumentItem.Quantity, DocumentItem.Price, DocumentItem.Total, Document.Number " +
			                              "FROM DocumentItem, Document, ProductTax, Product " +
			                              "WHERE DocumentItem.DocumentId = Document.Id AND DocumentItem.ProductId = ProductTax.ProductId " +
			                              "AND ProductTax.TaxId = '2' AND Number LIKE '19-200%' AND Document.Date = '" + mydate + "' " +
			                              "AND DocumentItem.ProductId = Product.Id";
			                System.out.println(SQL3);
			                try (ResultSet rs = stmt.executeQuery(SQL3)) {
			                    printerServices.printString("POS-Printer", "Item Name       Qty   Price   Total\n");
			                    printerServices.printString("POS-Printer", "------------------------------------------\n");
			                    ...
			                    while (rs.next()) {
			                        ...
			                        String itemLine = String.format("%-15s %4s %7s %7s", name, quantity, price, total);
			                        //printerServices.printString("POS-Printer", itemLine + "\n");
			                    }
			                }

			                // Printing the receipt footer with totals
			                printerServices.printString("POS-Printer", "\n------------------------------------------\n");
			                ...
			                printerServices.printString("POS-Printer", "User: " + UserName + "\n\n\n\n\n\n");

			                // Cut the paper and open the drawer
			                printerServices.printBytes("POS-Printer", cutP);
			                printerServices.printBytes("POS-Printer", openDrawer);
